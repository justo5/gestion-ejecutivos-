...
import { formatGoalMonth, formatYearMonth, DashboardGoal, PlanConfig } from './config';
import { Client, Executive } from './executives';
import { ClientViewBuilder } from './client-view-builder';
...
// Arma el view-model del dashboard (KPIs, series de tendencia, paneles de
// barras y el detalle de clientes de cada uno) a partir de los ejecutivos y
// sus clientes. Es lógica pura: la página solo la invoca y reparte el
// resultado a los componentes hijos.
@Injectable({ providedIn: 'root' })
export class DashboardViewBuilder {
  constructor(private viewBuilder: ClientViewBuilder) {}
  ...
  private statusGroupColor(label: string): string {
    return DashboardViewBuilder.STATUS_COLORS[label] ?? OTHER_COLOR;
  }
